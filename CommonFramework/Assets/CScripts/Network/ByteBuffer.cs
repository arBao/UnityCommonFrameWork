...
public class ByteBuffer
{
	...
	public void WriteBytesByLength(byte[] v, int len)
	{
		if (len > v.Length)
			len = v.Length;
		writer.Write(len);
	...
	public void WriteBytesWithoutLength(byte[] v)
	{
		writer.Write(v);
	}

	public void WriteBytesWithoutLength(byte[] v, int len)
	{
		if (len > v.Length)
			len = v.Length;
		writer.Write(v, 0, len);
	...
	{
		WriteBytesWithoutLength(strBuffer.buffer);
	}
	...
	public LuaByteBuffer ToLuaBuffer()
	{
		writer.Flush();
